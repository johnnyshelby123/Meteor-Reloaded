 ...
 */
 ...
 public class TitleScreenCredits {
     ...
     private static void add(MeteorAddon addon) {
         ...
         // Modified client name to "Meteor Client Reloaded"
         credit.text.append(Text.literal(addon == MeteorClient.ADDON ? "Meteor Client Reloaded" : addon.name).styled(style -> style.withColor(addon.color.getPacked())));
         credit.text.append(Text.literal(" by ").formatted(Formatting.GRAY));
 
         // Modified to show "MANUS AI" as author for main addon
         if (addon == MeteorClient.ADDON) {
             credit.text.append(Text.literal("MANUS AI").formatted(Formatting.WHITE));
         } else {
